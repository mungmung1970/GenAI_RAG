# rag_chain.py (LangSmith + LCEL + Hybrid + 개선된 Reranker)
...
os.environ["LANGCHAIN_PROJECT"] = os.getenv("LANGSMITH_PROJECT", "rag_lcel")
...
DENSE_WEIGHT = 0.7
LEX_WEIGHT = 0.3
TOP_N_AFTER_RERANK = 5  # 최종 선택 보장 문서 수
...
def hybrid_retrieve_with_rerank(question: str, k: int = 5):
    ...
    for c in final:
        m = c["meta"]
        header = f"chunk_id={c['chunk_id']} | page={m.get('page')} | source={m.get('source_file')} | section={m.get('section')}"
